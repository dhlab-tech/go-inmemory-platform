...
func TestCache_Update(t *testing.T) {
	c := projection.NewCache[*V](map[string]*V{})
	id := primitive.NewObjectIDFromTimestamp(time.Now())
	c.Add(context.Background(), &V{Id: id})
	c.Update(context.Background(), id.Hex(), &V{
		Name:   &name1,
		Number: &number1,
		Slice:  slice1,
		Map:    map1,
	...
	assert.Nil(t, v.Ver)
	c.Update(context.Background(), id.Hex(), &V{}, []string{})
	v, f = c.Get(context.Background(), id.Hex())
	assert.Equal(t, true, f)
	assert.NotNil(t, v.Name)
	assert.Equal(t, name2, *v.Name)
	...
	assert.NotNil(t, v.Map)
	assert.Equal(t, map2, v.Map)
	c.Update(context.Background(), id.Hex(), &V{}, []string{"name", "number", "slice", "map"})
	v, f = c.Get(context.Background(), id.Hex())
	assert.Equal(t, true, f)
	assert.Nil(t, v.Name)
...
// TestCache_Get tests cases:
// 1) that the method returns a copy of stored data
func TestCache_Get(t *testing.T) {
	...
	c.Update(context.Background(), id.Hex(), &V{S: S{CS: C{Name: name2}}}, []string{})
	v, f = c.Get(context.Background(), id.Hex())
