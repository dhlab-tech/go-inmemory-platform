...
import (
	...
	"github.com/dhlab-tech/go-inmemory-platform/pkg/mongo/inmemory"
	"github.com/dhlab-tech/go-inmemory-platform/pkg/mongo"
)

// bson.D built from Go maps has non-deterministic key order; normalize for assertions.
func sortBSONDMapField(d bson.D) bson.D {
	if d == nil {
		return nil
	}
	...
	for i := range out {
		...
		inner, ok := out[i].Value.(bson.D)
		if !ok {
			continue
		}
		cp := make(bson.D, len(inner))
		copy(cp, inner)
...
func TestProcessor_PrepareUpdate(t *testing.T) {
	...
	assert.Equal(t, sortBSONDMapField(expectedSet), sortBSONDMapField(set))
}
...
type Image struct {
	...
	P
	R
	Name   *string `json:"name" bson:"name" indexes:"sorted:title:from,suffix:title:from"`                   // file name
	Orig   *string `json:"orig" bson:"orig" indexes:"inverse_unique:origWidthHeight:from,inverse:orig:from"` // original image id
	Width  *int    `json:"width" bson:"width" indexes:"inverse_unique:origWidthHeight:from"`                 // image width
	Height *int    `json:"height" bson:"height" indexes:"inverse_unique:origWidthHeight:from"`               // image height
