...
type inverseIndex[T domain.Entity] struct {
	sync.RWMutex
...
func NewInverseIndex[T domain.Entity](
	data map[string][]string,
	nilData []string,
	cache Cache[T],
	from []string,
...
func (s *inverseIndex[T]) Update(ctx context.Context, id string, updatedFields T, removedFields []string) {
	s.Lock()
	defer s.Unlock()
...
// Delete ...
func (s *inverseIndex[T]) Delete(ctx context.Context, id string) {
	s.Lock()
	defer s.Unlock()
