...
// d is the entity constraint for Mongo-backed projections (same as [domain.Entity]).
type d = domain.Entity

type (
	// Cache re-exports the shared projection cache.
	Cache[T domain.Entity] = projection.Cache[T]
	// CacheWithEventListener re-exports the projection bundle (cache + indexes + await notify).
	CacheWithEventListener[T domain.Entity] = projection.CacheWithEventListener[T]
	// StreamEventListener re-exports the stream listener interface.
	StreamEventListener[T domain.Entity] = projection.StreamEventListener[T]
	// EventListener re-exports the event listener interface.
	EventListener[T domain.Entity] = projection.EventListener[T]
	// Notify re-exports the await-notify helper interface.
	Notify[T domain.Entity] = projection.Notify[T]
	// Notifier re-exports the notifier implementation type.
...
// Index and projection constants match [projection] names.
const (
...
// Shared projection-side types used by indexes.
type (
	M         = projection.M
...
// Constructors delegate to [projection]; kept for backward-compatible import paths.
...
// Listener is the concrete listener type from projection.
type Listener[T d] = projection.Listener[T]
...
type AddCallbackListener[T d] = projection.AddCallbackListener[T]
...
type UpdateCallbackListener[T d] = projection.UpdateCallbackListener[T]
...
type DeleteCallbackListener[T d] = projection.DeleteCallbackListener[T]
