...
// The cache is synchronized with durable storage via a change stream or logical replication.
type Cache[T domain.Entity] interface {
...
// cache ...
type cache[T domain.Entity] struct {
	Idx
	data map[string]T
}
...
func (c *cache[T]) Update(ctx context.Context, id string, updatedFields T, removedFields []string) {
	c.Lock()
	defer c.Unlock()
...
// Delete ...
func (c *cache[T]) Delete(ctx context.Context, id string) {
	c.Lock()
	defer c.Unlock()
	delete(c.data, id)
	c.deleteByID(id)
