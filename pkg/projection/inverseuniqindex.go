...
type inverseUniqueIndex[T domain.Entity] struct {
	sync.RWMutex
...
func NewInverseUniqIndex[T domain.Entity](
	data map[string]string,
	cache Cache[T],
	field []string,
	to *string,
...
// Update ...
func (s *inverseUniqueIndex[T]) Update(ctx context.Context, id string, updatedFields T, removedFields []string) {
	s.Lock()
	defer s.Unlock()
...
// Delete ...
func (s *inverseUniqueIndex[T]) Delete(ctx context.Context, id string) {
	s.Lock()
	defer s.Unlock()
