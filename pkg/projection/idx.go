...
// Idx provides bidirectional mapping between entity IDs and their index positions.
// It maintains both ID->index and index->ID mappings for efficient lookups.
type Idx struct {
	sync.RWMutex
	maxIdx       *atomic.Int64
...
// GetIDByIndex retrieves an entity ID by its index position.
...
// GetIndexByID retrieves the index position of an entity by its ID.
...
// NewIdx creates a new Idx instance with the provided initial state.
func NewIdx(
	maxIdx *atomic.Int64,
	itemsByIndex map[int64]string,
	indexByID map[string]int64,
