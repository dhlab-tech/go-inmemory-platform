...
type UpdateSuffix[T domain.Entity] struct {
	M
	cache Cache[T]
	from  []string
	to    *string
...
func NewUpdateSuffix[T domain.Entity](index M, cache Cache[T], from []string, to *string) SuffixIndex[T] {
	return &Suffix[T]{
		M:     index,
		cache: cache,
		from:  from,
...
func (s *UpdateSuffix[T]) Update(ctx context.Context, id string, updatedFields T, removedFields []string) {
	it, ok := s.cache.Get(context.Background(), id)
	if !ok {
		return
	}
...
func (s *UpdateSuffix[T]) Delete(ctx context.Context, _id string) {
}
...
type Suffix[T domain.Entity] struct {
	M
	cache Cache[T]
	from  []string
	to    *string
...
func NewSuffix[T domain.Entity](index M, cache Cache[T], from []string, to *string) SuffixIndex[T] {
	return &Suffix[T]{
		M:     index,
		cache: cache,
		from:  from,
...
// Update ...
func (s *Suffix[T]) Update(ctx context.Context, id string, updatedFields T, removedFields []string) {
	it, ok := s.cache.Get(context.Background(), id)
	if !ok {
		return
	}
...
// Delete ...
func (s *Suffix[T]) Delete(ctx context.Context, _id string) {
	it, ok := s.cache.Get(context.Background(), _id)
	if !ok {
		return
	}
	from := updateStringFieldValuesByName(it, s.from)
...
// SuffixTree is the trigram backing store used by [M].
...
func NewSuffixIndex[T domain.Entity](cache Cache[T], btreeDegree int, from []string, to *string) (SuffixIndex[T], SuffixIndex[T]) {
...
func BuildM[T domain.Entity](cache Cache[T]) M {
	sorterIntersector := NewIntersect()
	suffixPool := NewPool()
	return NewM(
		cache,
