...
// Intersect provides set operations (intersection, union, left outer join) for slices.
type Intersect struct {
}

// Intersect finds the intersection of multiple string slices.
// Returns elements that appear in in1 and in all other slices.
...
// IntersectInt finds the intersection of two integer slices.
...
// Union finds the union of two string slices (all unique elements from both slices).
...
// UnionInt finds the union of two integer slices (all unique elements from both slices).
...
// NewIntersect creates a new Intersect instance for performing set operations.
